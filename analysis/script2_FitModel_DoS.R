...
change_trend_fn <- function(mat){
  ...
  #OLS_regression_samples <- 100*(exp(OLS_regression_samples)-1) # Convert to percent change per year
...
tiff(filename = "output/figures_and_tables/model_results/global_trajectory.tif", width = 5, height = 4, units = "in", res = 300)
print(global_plot)
dev.off()
...
png("output/figures_and_tables/model_results/colony_dynamics_fitted.png", units = "in", res = 1000, width = 20, height = 20)
print(p1)
dev.off()

pdf("output/figures_and_tables/model_results/colony_dynamics_fitted.pdf", width = 20, height = 20)
print(p1)
dev.off()
